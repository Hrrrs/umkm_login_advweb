...
function getConfig() {
  ...
  return {
    ...
    // Keep this low to respect Clever Cloud limits (often 5 connections/user)
    connectionLimit: Number(env.MYSQL_POOL_LIMIT || 2),
...
async function init() {
  ...
  try {
    ...
    // Optional test connection (disabled by default to avoid consuming a slot)
    if ((process.env.TEST_MYSQL_ON_INIT || '0').toLowerCase() === '1') {
